...
class UltronWebWindow(QMainWindow):
    ...
    _stats_sig = pyqtSignal(float, float)
    ...
    def __init__(self, face_path: str = "face.png"):
        ...
        self._stats_sig.connect(self._on_stats)
    ...
    def _on_stats(self, cpu: float, mem: float):
        # The dashboard's own receive(m) already knows how to render m.cpu/m.mem —
        # reuse it instead of duplicating that DOM logic here. This is the local-HUD
        # equivalent of what the remote WebSocket client gets automatically; the local
        # page's connect() intentionally skips the WebSocket, so nothing else was
        # ever calling receive() with live stats before now.
        js = f"if (typeof receive === 'function') receive({{cpu: {cpu:.0f}, mem: {mem:.0f}}});"
        self._eval_js(js)
...
class UltronUI:
    ...
    def update_stats(self, cpu: float, mem: float):
        self._win._stats_sig.emit(cpu, mem)
