...
class UltronLive:
    ...
    def _enqueue_mic_audio(self, item: dict) -> None:
        """Push a mic chunk onto out_queue, making room by dropping the oldest
        buffered chunk if the network side (send_realtime_input) has fallen
        behind and the queue is full — instead of raising QueueFull from an
        unawaited call_soon_threadsafe callback, which silently dropped every
        subsequent chunk (total mic loss) while spamming the log."""
        try:
